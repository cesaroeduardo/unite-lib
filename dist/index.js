...
// src/moves.ts
var SLOT_IDS = ["s11", "s12", "s13", "s21", "s22", "s23", "s24"];
function buildMoves() {
  const moves2 = [];
  for (const pokemon of pokemons_default) {
    for (const slotId of SLOT_IDS) {
      const key = `move_${slotId}`;
...
function getPokemonsByTag(tag) {
  return pokemons_default.filter((p) => p.tags.includes(tag));
}
function getActivePokemons() {
  return pokemons_default.filter((p) => p.active);
}
