...
__export(src_exports, {
  ...
  POKEMON_MOVE_SLOT_IDS: () => POKEMON_MOVE_SLOT_IDS,
  Tag: () => Tag,
  battleItems: () => battleItems_default,
  getActivePokemons: () => getActivePokemons,
  getBattleItemById: () => getBattleItemById,
  getBattleItemName: () => getBattleItemName,
  getExclusiveHeldItems: () => getExclusiveHeldItems,
  getHeldItemById: () => getHeldItemById,
  getHeldItemName: () => getHeldItemName,
  getHeldItemsByPokemon: () => getHeldItemsByPokemon,
  ...
  getPokemonSkillNames: () => getPokemonSkillNames,
  ...
  resolveMoveSlot: () => resolveMoveSlot,
...
function getImageUrl(pokemon, imageKey, options) {
  ...
  const path = typeof raw === "string" ? raw : typeof raw === "object" && raw != null && "image" in raw ? raw.image : "";
  if (path === "") return "";
...
function getPokemonSkillNames(pokemon) {
  const merged = {};
  for (const slotId of POKEMON_MOVE_SLOT_IDS) {
    const key = `move_${slotId}`;
    const v = pokemon.images[key];
    if (v && typeof v === "object" && "name" in v && v.name != null && v.name !== "") {
      merged[slotId] = v.name;
    }
  }
  return { ...merged, ...pokemon.skillNames };
}
function resolveMoveSlot(pokemon, slotId) {
  const key = `move_${slotId}`;
  const raw = pokemon.images[key];
  if (raw == null || raw === "") return null;
  const names2 = getPokemonSkillNames(pokemon);
  const fallbackName = names2[slotId] ?? `${pokemon.name} (${slotId})`;
...
function getBattleItemById(id) {
  return battleItems_default.find((b) => b.id === id);
}
function getHeldItemById(id) {
  return heldItems_default.find((h) => h.id === id);
}
function getHeldItemsByPokemon(pokemonName) {
  return heldItems_default.filter(
    (h) => !h.exclusive || h.exclusive === pokemonName
  );
}
function getExclusiveHeldItems() {
  return heldItems_default.filter((h) => h.exclusive != null);
}
...
function buildMoves() {
  ...
  for (const pokemon of pokemons_default) {
    for (const slotId of POKEMON_MOVE_SLOT_IDS) {
      ...
      const pokemonId = match ? match[1] : getPokemonSlug(pokemon);
      moves2.push({
        pokemonId,
        slotId,
        name: resolved.name,
